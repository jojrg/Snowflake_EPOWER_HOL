# Haeufig gestellte Fragen zum ePulse VPP Programm

## Allgemeine Fragen

### Was genau ist ein Virtuelles Kraftwerk?

Ein Virtuelles Kraftwerk (VPP - Virtual Power Plant) buendelt viele kleine, dezentrale Energiequellen und -speicher zu einer grossen, steuerbaren Einheit. Bei ePulse verbinden wir tausende Batteriespeicher unserer Kunden, um gemeinsam zur Netzstabilitaet beizutragen - aehnlich wie ein grosses Kraftwerk, nur verteilt auf viele Haushalte.

### Warum braucht das Stromnetz ePulse?

Mit dem Ausbau erneuerbarer Energien schwankt das Stromangebot staerker. Wenn die Sonne scheint und der Wind weht, gibt es oft zu viel Strom. Abends und bei Flaute hingegen zu wenig. ePulse hilft, diese Schwankungen auszugleichen:
- **Ueberschuss**: Speicher nehmen ueberschuessigen Strom auf
- **Mangel**: Speicher geben Strom ab

### Wie viele Kunden nehmen bereits teil?

Aktuell sind ueber **15.000 Haushalte** im ePulse Programm aktiv. Gemeinsam stellen wir eine Kapazitaet von mehr als **120 MWh** bereit - das entspricht der Leistung eines kleinen Gaskraftwerks.

---

## Teilnahme und Voraussetzungen

### Kann ich mit jedem Batteriespeicher teilnehmen?

Nein, fuer ePulse sind folgende Voraussetzungen noetig:
- EPOWER Batteriespeicher oder kompatibles System
- Mindestkapazitaet: 5 kWh
- Funktionierendes WLAN oder LAN
- Aktiver EPOWER Stromvertrag

### Brauche ich eine Solaranlage?

Nein, eine Solaranlage ist **nicht zwingend erforderlich**. Auch Speicher ohne PV-Anlage koennen teilnehmen. Allerdings profitieren Kunden mit Solar staerker, da sie ueberschuessigen Solarstrom einsetzen koennen.

### Was ist das ePulse Gateway?

Das Gateway ist ein kleines Geraet (ca. 12 x 8 x 3 cm), das Ihren Batteriespeicher mit unserem ePulse-System verbindet. Es:
- Empfaengt Steuersignale von der EPOWER Leitwarte
- Uebertraegt Telemetriedaten (Ladestand, Leistung)
- Arbeitet verschluesselt und sicher

**Wichtig**: Das Gateway wird Ihnen kostenlos bereitgestellt.

### Kostet die Teilnahme etwas?

Nein, die Teilnahme ist **kostenfrei**. Im Gegenteil: Sie erhalten eine jaehrliche Verguetung von bis zu 150 EUR.

---

## Betrieb und Steuerung

### Wann wird mein Speicher angesteuert?

Typische Einsatzzeiten:
- **Morgens 6-9 Uhr**: Hohe Nachfrage (Positive Regelenergie)
- **Mittags 11-14 Uhr**: Solarpeak (Negative Regelenergie)
- **Abends 17-21 Uhr**: Spitzenlast (Positive Regelenergie)

Durchschnittlich wird Ihr Speicher **2-4 mal pro Woche** aktiviert, jeweils fuer 15-45 Minuten.

### Beeintraechtigt das meinen Eigenverbrauch?

**Nein!** Ihr Eigenverbrauch hat immer Prioritaet:
- Mindestens 80% der Kapazitaet sind fuer Sie reserviert
- Bei niedrigem Ladestand wird Ihr Speicher nicht angesteuert
- Sie koennen jederzeit in der App pausieren

### Was passiert bei einem Stromausfall?

Bei einem Stromausfall wird Ihr Speicher **automatisch aus dem VPP genommen**. Die volle Kapazitaet steht dann fuer Ihre Notstromversorgung zur Verfuegung (sofern Ihr System Notstrom unterstuetzt).

### Kann ich die Teilnahme pausieren?

Ja, jederzeit ueber die EPOWER Solar App:
1. App oeffnen → ePulse → Einstellungen
2. "VPP pausieren" waehlen
3. Dauer festlegen (1 Stunde bis 7 Tage)

Waehrend der Pause erhalten Sie keine Verguetung, aber auch keine Ansteuerungen.

---

## Verguetung und Abrechnung

### Wie wird die Verguetung berechnet?

| Komponente | Berechnung |
|------------|------------|
| Grundverguetung | 50 EUR/Jahr (fix) |
| Leistungsverguetung | 0,05 EUR pro bereitgestellte kWh |
| Boersenpreisbonus | Bei negativen Strompreisen: 50% der Ersparnis |

### Wann wird die Verguetung ausgezahlt?

- **Quartalweise**: Leistungsverguetung
- **Jaehrlich (31.03.)**: Grundverguetung
- **Auszahlung**: Per Ueberweisung oder Gutschrift auf Stromrechnung

### Kann ich meine Einsaetze nachverfolgen?

Ja, in der EPOWER Solar App finden Sie:
- Kalenderuebersicht aller Aktivierungen
- Energie pro Einsatz (kWh)
- Aktueller Verguetungsstand
- Prognostizierte Jahresverguetung

---

## Sicherheit und Datenschutz

### Ist die Verbindung sicher?

Ja, alle Daten werden:
- Ende-zu-Ende verschluesselt uebertragen
- In deutschen Rechenzentren gespeichert
- Nur fuer den VPP-Betrieb verwendet

### Welche Daten werden uebertragen?

- Aktueller Ladestand (%)
- Aktuelle Lade-/Entladeleistung (kW)
- Systemstatus (online/offline)
- Keine persoenlichen Daten

### Wer hat Zugriff auf meinen Speicher?

Nur das automatisierte EPOWER VPP-System kann Steuerbefehle senden. Kein Mitarbeiter hat direkten Zugriff auf Ihren Speicher.

---

## Technische Fragen

### Das Gateway verbindet sich nicht - was tun?

1. WLAN-Signal am Gateway-Standort pruefen
2. Gateway neu starten (Stecker ziehen, 10 Sek. warten)
3. Router neu starten
4. Hotline kontaktieren: 0800 - 22 33 559

### Mein Speicher zeigt Fehlermeldungen seit ePulse

Das ePulse System prueft regelmaessig die Kommunikation. Manche Wechselrichter zeigen dies als "externe Verbindung" an. Dies ist normal und kein Fehler.

### Verkuerzt ePulse die Lebensdauer meiner Batterie?

Nein, im Gegenteil:
- Maximal 250 zusaetzliche Zyklen/Jahr
- Intelligente Steuerung vermeidet Tiefentladung
- Temperaturueberwachung schuetzt die Zellen

Studien zeigen: ePulse-Teilnehmer haben im Schnitt **5% laengere Batterielebensdauer** durch optimierte Ladezyklen.

---

## Vertragsdetails

### Was steht in den Teilnahmebedingungen?

Die wichtigsten Punkte:
1. Mindestlaufzeit: 12 Monate
2. Kuendigungsfrist: 4 Wochen zum Monatsende
3. Verguetungsanspruch bei min. 80% Verfuegbarkeit
4. Haftungsausschluss bei technischen Stoerungen

### Kann ich jederzeit kuendigen?

Nach der Mindestlaufzeit von 12 Monaten: Ja, mit 4 Wochen Frist.
Das Gateway muss innerhalb von 14 Tagen zurueckgesendet werden (Ruecksendelabel wird gestellt).

---

## Kontakt

**ePulse Kundenservice**
- Telefon: 0800 - 22 33 559 (kostenfrei)
- E-Mail: epulse@eon.de
- Chat: mein.eon.de/epulse

**Technischer Support**
- Telefon: 0800 - 22 33 560
- E-Mail: epulse-technik@eon.de

**Servicezeiten**: Mo-Fr 8-20 Uhr, Sa 9-16 Uhr

---

*Stand: Maerz 2024 | Version 2.1*
